# @egiftcard/token-lists (beta)

[![Tests](https://github.com/Egiftcard/token-lists/workflows/Tests/badge.svg)](https://github.com/Egiftcard/token-lists/actions?query=workflow%3ATests)
[![npm](https://img.shields.io/npm/v/@egiftcard/token-lists)](https://unpkg.com/@egiftcard/token-lists@latest/)

This package includes a JSON schema for token lists, and TypeScript utilities for working with token lists.

The JSON schema represents the technical specification for a token list which can be used in a dApp interface, such as the Egiftcard Interface.

## What are token lists?

Egiftcard Token Lists is a specification for lists of token metadata (e.g. address, decimals, ...) that can be used by any dApp interfaces that needs one or more lists of tokens.
...
Token list JSON must validate against the [JSON schema](https://json-schema.org/) in order to be used in the Egiftcard Interface.
Tokens on token lists, and token lists themselves, are tagged so that users can easily find tokens.

## JSON Schema $id

The JSON schema ID is [https://egiftcard.cc/tokenlist.schema.json](https://egiftcard.cc/tokenlist.schema.json)
...
import { schema } from '@egiftcard/token-lists'
import Ajv from 'ajv'
...
```typescript
import { TokenList, schema } from '@egiftcard/token-lists'
...
A snapshot of the Egiftcard default list encoded as a token list is found in [test/schema/bigexample.tokenlist.json](test/schema/bigexample.tokenlist.json).
